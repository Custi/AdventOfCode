...
let lastNode = null;
nodes.forEach((node, nodeName, map) => {
  node.childrenNames.forEach(cn => {
    const child = map.get(cn)
    child.parent = node;
...
function getChildWeight(node) {
  if (node.children.length === 0) {
    node.treeWeight = node.weight;
    return node.treeWeight;
  }

  node.treeWeight = node.children.reduce((acc, child) =>  acc + getChildWeight(child), node.weight);
  return node.treeWeight;
}

function printTree(node, level = 0) {
  const offset = '  ';
  console.log(`${offset.repeat(level)} ${node.name} (${node.weight}, ${node.treeWeight})`);
  node.children.forEach(n => printTree(n, level + 1));
}

function findProblemNode(node) {
  if (node.children.length === 0) return true;
  const isBalanced = node.children.every(c => c.treeWeight === node.children[0].treeWeight);
  if (isBalanced) {
    return node;
  }
  else {
    const weights = node.children.map(c => c.treeWeight);
    const min = Math.min(...weights);
    const max = Math.max(...weights);
    
    let wrongNodes = node.children.filter(c => c.treeWeight === min);
    if ( wrongNodes.length !== 1 ) {
      wrongNodes = node.children.filter(c => c.treeWeight === max);
    }

    return findProblemNode(wrongNodes[0]);
  }
}

getChildWeight(root);
const problem = findProblemNode(root);
const rightTreeWeight = problem.parent.children.filter(c => c !== problem)[0].treeWeight;
const diff = rightTreeWeight - problem.treeWeight;

console.log('Part 2: ', problem.weight + diff);